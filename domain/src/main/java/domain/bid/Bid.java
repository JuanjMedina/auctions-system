...
public class Bid {
    ...
    private Bid(UUID id, UUID auctionId, UUID bidderId, BigDecimal amount,
                boolean autoBid, BigDecimal maxAutoAmount, BidStatus status, Instant createdAt) {
        this.id             = id;
    ...
    public static Bid create(UUID auctionId, UUID bidderId, BigDecimal amount,
                             boolean autoBid, BigDecimal maxAutoAmount) {
        ...
        if (autoBid && (maxAutoAmount == null || maxAutoAmount.compareTo(amount) < 0))
            throw new IllegalArgumentException("El monto maximo de auto-puja debe ser >= al monto de la puja");
        if (!autoBid && maxAutoAmount != null)
            throw new IllegalArgumentException("Solo una auto-puja puede tener monto maximo");
        return new Bid(UUID.randomUUID(), auctionId, bidderId, amount,
                autoBid, maxAutoAmount, BidStatus.ACTIVE, Instant.now());
    }

    // Factory method: reconstruir desde persistencia
    public static Bid reconstitute(UUID id, UUID auctionId, UUID bidderId, BigDecimal amount,
                                   boolean autoBid, BigDecimal maxAutoAmount,
                                   BidStatus status, Instant createdAt) {
    ...
    public BigDecimal getMaxAutoAmount() { return maxAutoAmount; }
